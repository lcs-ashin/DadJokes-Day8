...
struct ContentView: View {
    ...
    var body: some View {
        VStack {
            ...
            Button(action: {
                ...
                // so that it runs saynchronously
                // NOTE: Button's action normaly expects synchronous code.
                Task {
                    await loadNewJoke()
                }
            ...
            //                 to identify each joke using the text of the joke itself
        ...
        .task {
            ...
            // By typing "await" we are acknowlodging that we know this
    ...
    // We must mark the function as "async" so that it can be asynchronously witch
    // means it may be run at the same time as other tasks.
    // This is the function definition (it is where the computer "learns" what it takes to load
    // a new joke.)
